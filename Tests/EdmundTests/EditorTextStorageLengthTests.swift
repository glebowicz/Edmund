...
/// Checks text, UTF-16 lengths, and cache reuse across edits.
@Suite("Editor text storage — length and string cache")
struct EditorTextStorageLengthTests {
    ...
    @MainActor
    @Test("appending storage to itself keeps text and notifications current",
          arguments: [false, true])
    func appendingSelfKeepsStringCurrent(primeCache: Bool) {
        let source = "abc 🌍"
        let ts = storage(source)
        if primeCache { #expect(ts.string == source) }
        let observer = StorageStringObserver()
        ts.delegate = observer

        ts.append(ts)

        let expected = source + source
        #expect(observer.strings == [expected])
        #expect(ts.string == expected)
        #expect(ts.length == (expected as NSString).length)
    }

    @MainActor
    @Test("replacing a range with the storage itself keeps text and notifications current",
          arguments: [false, true])
    func replacingWithSelfKeepsStringCurrent(primeCache: Bool) {
        let source = "abc 🌍"
        let ts = storage(source)
        if primeCache { #expect(ts.string == source) }
        let observer = StorageStringObserver()
        ...
        ts.replaceCharacters(in: NSRange(location: 0, length: 3), with: ts)
...
/// Reads during notification so the cache must be invalidated before `edited`.
private final class StorageStringObserver: NSObject, NSTextStorageDelegate {
    var strings: [String] = []
    ...
    func textStorage(_ textStorage: NSTextStorage,
        ...
        if editedMask.contains(.editedCharacters) {
            strings.append(textStorage.string)
        }
    }
}
